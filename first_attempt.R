...
# ggsave("single_pairs.png", height = 20, width = 27, units = "cm")
...
####### - Only uses 1 parameter mclust
####### - Only one iteration/cycle
########################################################################
########################################################################
########################################################################
########################################################################
...
for (variable in 1:ncol(test.data)) {
  ...
  sigma <- matrix(model.mclust$parameters$variance$sigma, ncol = ncol(test.data))
  ...
  for (row in position) {
    ...
    test <- condMVN(mean=mu.it, sigma=sigma, dependent=dependent, given=given,
            X.given=x.given)
    
    test.data[row,variable] <- rnorm(1,test$condMean, test$condVar)
  }
...
# True/False location
data.missing <- matrix(is.na(dataset.mcar.or), ncol =ncol(dataset.mcar.or))

imp1 <- test.data

imp1[,][!data.missing] <- NA
